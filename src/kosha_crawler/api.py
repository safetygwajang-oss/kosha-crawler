...
class KoshaAPI:
    ...
    def list_media(self, page: int = 1, rows: int | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            log.info(f"  [API] list page={page} status={r.status_code} time={elapsed:.1f}s size={len(r.content)}B")
            log.info(f"  [API] preview: {r.text[:300]}")
            r.raise_for_status()
            return r.json().get("payload", {})
        except Exception as e:
            elapsed = time.time() - t0
            log.error(f"  [API] list page={page} FAILED after {elapsed:.1f}s: {e}")
            raise
